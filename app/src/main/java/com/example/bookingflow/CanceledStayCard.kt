...
@Preview
@Composable
fun CanceledStayCard(modifier: Modifier = Modifier, index: Int = 0, listSize: Int = 0) {
    Column(
        modifier = modifier
            .fillMaxWidth()
...
//            .clickable { navController.navigate(HorizontalScrollTicketScreen(index = index)) }
        ...
        UpcomingImageCard(
            imageCardColor = Color(0xFF464646),
            bookingStatus = "Cancelled",
            bookingStatusColor = Color(0xFFF2503F),
            cardImage = painterResource(id = R.drawable.bus_trip_img),
            upcomingCardName = "Stay to Goa"
        )
        if (index == listSize - 1) {
